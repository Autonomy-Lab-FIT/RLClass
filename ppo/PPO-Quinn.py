...
class PPO_agent(object):
    def __init__(self, **kwargs):
        ...
        ''' Build Actor '''
        self.actor = Actor(state_size, action_size, hidden_size).to(self.dvc)
        self.actor_optimizer = torch.optim.Adam(self.actor.parameters(), lr=self.a_lr)

        '''Build Critic'''
        self.critic = Critic(state_size, hidden_size).to(self.dvc)
    ...
    def train(self):
        ...
        for i in range(self.K_epochs):
            ...
            for i in range(a_optim_iter_num):
                ...
                a_loss = (-torch.min(surr1, surr2) - self.entropy_coef * dist_entropy).mean()
